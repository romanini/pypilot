...
int WifiServo::process_packet(uint8_t *in_buf)
{
    if(packet_count < 255)
        packet_count++;
...
void WifiServo::reset()
{
    send_value(RESET_CODE, 0);
}

void WifiServo::disengage()
{
    send_params();
    send_value(DISENGAGE_CODE, 0);
}

void WifiServo::reprogram()
{
